...
class AliexpressWeb(Web):
    ...
    def buscar_productos(self, categoria, num_productos,atributos_en_profundidad,atributos_a_extraer, log_callback=None):
        """
        Realiza la búsqueda de productos en la página de Aliexpress.
        ...
        try:
            ...
            sleep(1)
            self.driver.get(url)
            self.driver.find_element(By.TAG_NAME,'body').send_keys(Keys.END)#Subimos y bajamos para que cargen todos los elementos de la pagina
        ...
        while Numero_Productos != num_productos:
            ...
            for i in range(len(elementosList)):
                ...
                except:#Esto es necesario porque hay veces que dejan de aparecer todos los elemenos cuando estamos de la mitad para abajo
                    self.driver.refresh()
                    sleep(1)
                    self.driver.find_element(By.TAG_NAME,'body').send_keys(Keys.END)
                    sleep(1)
                ...
                try:
                    ...
                    print("Fallo al guardar el elemento")
                if log_callback is not None:
                    try:
                        sleep(1)
    ...
    def obtener_url_aliexpress(self, categoria):
        """
        Obtiene la URL correspondiente a una categoría de productos en Aliexpress.

        Args:
            categoria (str): Categoría de productos.

        Returns:
            str: URL de la categoría en Aliexpress.
        """
        categorias_urls = {
